...
(defn sidebar-contents
  ...
  (let [{:keys [cljdoc.doc/articles cljdoc.doc/links]} (-> cache-bundle :version :doc)
    ...
    [;; Upgrade notice
     ...
     (when (seq links)
       [:div.mb4.js--links
        (layout/sidebar-title "Links" {:separator-line? (seq links)})
        [:div.mv3 (articles/doc-tree-links version-entity links)]])
